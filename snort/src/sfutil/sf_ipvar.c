...
#ifdef WANG
    memset(ret->src_ip,0,sizeof(ret->src_ip));
#endif
...
//            printf("the tok %s ip is %s\n",tok,inet_ntoa(&(tmp_var->head->ip->addr)));
...
#ifdef WANG
        memset(temp->src_ip,0,sizeof(temp->src_ip));
        strcpy(temp->src_ip,idx->src_ip);
#endif
