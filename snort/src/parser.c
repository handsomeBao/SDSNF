...
#ifdef WANG
int InitOfpRule();
int ParseOfpRule(SnortConfig *sc, SnortPolicy *p,char *args,
			 RuleType rule_type,ListHead* list);
char* OfpToSnort(OfpHead ofp_head);
#endif
                               ...
                               char *rule_opts, RuleType rule_type, int protocol)
{
    ...
    else
    {
        ...
        for (i = 0; i < num_toks; i++)
        {
            ...
            {
//             	printf("the rule_options[%d].name is %s and the opts[0] is %s\n",j,rule_options[j].name,opts[0]);
            ...
            if (rule_options[j].name == NULL)
            {
                ...
                {
//                	printf("now the dopt->keyword is %s and the opts[0] is %s\n",dopt->keyword,opts[0]);
...
#ifdef WANG
    int err = ParseOfpRule(sc,p,args,RULE_TYPE__ALERT,&sc->Alert);
    if(err==-1)
    {
    	printf("ParseOfpRule failed!\n");
    }
    OfpToSnort(MyHead);
#endif
}
...
/****************************************************************************
 * Name   : InitOfpRule
 * Purpose: Init the OfpRuleHead
 * Arguments: None
 * Returns:  Succeed:1  Failed: -1
 * Notes:    None
 *****************************************************************************/
int InitOfpRule()
{
	MyHead = (OfpHead)malloc(sizeof(OfpRuleHead));
	if(MyHead==NULL)
	{
		printf("in InitOfpRule, MyHead malloc error!\n");
		free(MyHead);
		return -1;
	}
	MyHead->RuleNode = (OfpNode)malloc(sizeof(OfpRuleNode));
	if(MyHead->RuleNode==NULL)
	{
		printf("in InitOfpRule, MyHead->RuleNode malloc error!\n");
		free(MyHead);
		return -1;
	}
	MyHead->count = 0;
//	MyHead->RuleNode->next = (OfpNode)malloc(sizeof(OfpRuleNode));
	CurrentNode = MyHead->RuleNode;
	if(CurrentNode==NULL)
	{
		printf("in InitOfpRule, the first node malloc error!\n");
		free(MyHead);
		return -1;
	}
...
/****************************************************************************
 * Name   : ParseOfpRule
 * Purpose: Convert the snort rule to Ofp rule
 * Arguments: SnortConfig *sc
 * 			  SnortPolicy *p
 *  	      char*    args -- the remaining of the snort rule to be parsed
 *  	      RuleType type -- Specify the type of snort rule
 *  	      ListHead* list
 * Returns: void function
 * Notes:    None
 *****************************************************************************/
			 ...
			 RuleType rule_type,ListHead* list)
{
	if(MyHead==NULL)
	{
		printf("in ParseOfpRule, MyHead is NULL! Are you kidding me?\n");
		return -1;
	}
	memset(CurrentNode,0,sizeof(OfpRuleNode));
	CurrentNode->next = NULL;
	char **toks = NULL;
	char *roptions = NULL;
	int num_toks = 	0;
	...
	PortVarTable *portVarTable = p->portVarTable;
	PortTable *nonamePortVarTable = p->nonamePortVarTable;
	MyPortList src_port_list;
	MyPortList dst_port_list;
	src_port_list.length = 0;
	src_port_list.port = NULL;
	dst_port_list.length = 0;
	dst_port_list.port = NULL;
	int protocol;
	/*for the priority*/
	...
	toks = mSplit(args," \t", 7, &num_toks, '\\');
	/*******************************************************************************/
	/***for example : tcp !192.168.0.1/24 any -> any 21 (content"USER";msg:"FTP";)***/
	/*******************************************************************************/
	/* proto ip port dir ip port options*/
	if(num_toks<6)
		ParseError("Bad rule for Ofp Parse:%s",args);
	if(num_toks ==7 )
		roptions = toks[6];
	/* get the protocol*/
	protocol = GetRuleProtocol(toks[0]);

	/* Now try to get the source IP*/
	ProcessIP(sc,toks[1],&test_rtn,SRC,0);



	/* now deal with the src ports in RTN*/
	if ((strcasecmp(toks[2], RULE_DIR_OPT__DIRECTIONAL) == 0) ||
       (strcasecmp(toks[2], RULE_DIR_OPT__BIDIRECTIONAL) == 0))
    {
       ParseError("Port value missing in rule!");
	...
	/* Now try to get the dest IP*/
	ProcessIP(sc, toks[4], &test_rtn, DST, 0);



    /* for the RTN dst ports*/
    if (ParsePortList(&test_rtn, portVarTable, nonamePortVarTable,
                      toks[5], protocol, 1 /* =dst port */ ))
    {
        ParseError("Bad destination port: '%s'", toks[5]);
	...
	/*the ip and netmask are stored in the test_rtn
	 * the ip is stored in test_rtn.sip->head->src_ip ,   char[50]
	 * the netmask is stored in the test_rtn.sip->head->ip->bits, we need to minus 96 for ipv4,     uint16_t
	 */
	sfip_var_t *temp_sip = test_rtn.sip;
	sfip_var_t *temp_dip = test_rtn.dip;
	int sip_head_count = temp_sip->head_count;
	int sip_neghead_count = temp_sip->neg_head_count;
	int sip_total_count = sip_head_count + sip_neghead_count;
	sfip_node_t* sip_head_head = temp_sip->head;
	sfip_node_t* sip_head_neghead = temp_sip->neg_head;

	int dip_head_count = temp_dip->head_count;
	int dip_neghead_count = temp_dip->neg_head_count;
	int dip_total_count = dip_head_count + dip_neghead_count;
	sfip_node_t* dip_head_head = temp_dip->head;
	sfip_node_t* dip_head_neghead = temp_dip->neg_head;

    /* now change the RTN portlist to our list*/
    /*  for the src portlist in our list*/
	PortObject *temp = test_rtn.src_portobject;
	SF_LIST  *temp_list = temp->item_list;
	PortObjectItem * temp_item;
	int my_count = 0;
	for(temp_item=(PortObjectItem*)sflist_first(temp_list);
		temp_item!=0;
		temp_item=(PortObjectItem*)sflist_next(temp_list))
	{
		src_port_list.port = (MyPort*)realloc(src_port_list.port,sizeof(MyPort)*(src_port_list.length+1));
		src_port_list.port[src_port_list.length].type = temp_item->type;
		src_port_list.port[src_port_list.length].port  = temp_item->lport;
		src_port_list.port[src_port_list.length].hport = temp_item->hport;
		src_port_list.length += 1;
		my_count++;
	}
	/* for the dst portlist in our list*/
	temp = test_rtn.dst_portobject;
	temp_list = temp->item_list;
	for(temp_item=(PortObjectItem*)sflist_first(temp_list);
		temp_item!=0;
		temp_item=(PortObjectItem*)sflist_next(temp_list))
	{
		dst_port_list.port = (MyPort*)realloc(dst_port_list.port,sizeof(MyPort)*(dst_port_list.length+1));
		dst_port_list.port[dst_port_list.length].type = temp_item->type;
		dst_port_list.port[dst_port_list.length].port  = temp_item->lport;
		dst_port_list.port[dst_port_list.length].hport = temp_item->hport;
		dst_port_list.length += 1;
	}

	/* TODO: you need to deal with the option part for the struct Proprietary */
	Proprietary *TemPro = NULL;
	TemPro = (Proprietary*)malloc(sizeof(Proprietary));
	memset(TemPro,0,sizeof(Proprietary));
	TemPro->length = 0;
	if(num_toks==6)
	{
		/* we don't have the options part*/
		TemPro->flags = 0;
	}
	else
	{
		/* we have the options part*/
		TemPro->flags = 1;
	}
	if(TemPro->flags == 1)
	{
	    ...
	    rule_opts++;
	    rule_opts[strlen(rule_opts) - 1] = '\0';
	    pro_toks = mSplit(rule_opts,";",0,&pro_tok_num,'\\');
	    for(int i = 0; i < pro_tok_num ; i++)
	    {
	    	char **opts = NULL;
	    	int num_opts;
	    	int j;
	    	/* temp var to store the found keyword and msg */
	    	char keyword_found[20];
	    	char msg_found[1280];

	    	memset(keyword_found,'\0',sizeof(keyword_found));
	    	memset(msg_found,'\0',sizeof(msg_found));
	    	opts = mSplit(pro_toks[i],":",2,&num_opts,'\\');

//	    	printf("now the opts[0] is %s and the opts[1] is %s\n",opts[0],opts[1]);
	    	/* first search from the rule_options */
	    	for(j = 0; rule_options[j].name!=NULL;j++)
	    	{
	    		if(strcasecmp(opts[0],rule_options[j].name) == 0)
	    		{
	    			strcpy(keyword_found,rule_options[j].name);
	    			if(num_opts==2)
	    			{
	    				strcpy(msg_found,opts[1]);
	    			}
	    		}
	    	}
	    	/*we don't find it from the rule_option, so we need to find it in option plugin*/
	    	if(rule_options[j].name==NULL)
	    	{
	    		RuleOptConfigFuncNode *dopt = rule_opt_config_funcs;
	    		for(; dopt != NULL; dopt=dopt->next)
	    		{
	    			if(strcasecmp(opts[0],dopt->keyword)==0)
	    			{
	    				strcpy(keyword_found,dopt->keyword);
	    				if(num_opts==2)
	    				{
	    					strcpy(msg_found,opts[1]);
	    				}
	    	...
	    	/* prove that we dont find your keyword, maybe it's from a preprocessor option or wrong keyword?*/
	    	if(keyword_found==NULL)
	    	{
	    		printf("we dont find your keyword %s, maybe it's from a preprocessor option or wrong keyword?",keyword_found);
	    	}
	    	/* store the keyword and msg to TemPro*/
	    	else
	    	{
	    		/* first we need to search if there is the same keyword already before*/
	    		/* if we find this situation, we just add the msg part to the existed one. Seperately by symbol '$%^&' */
	    		int my_flag = 0;
	    		for(int i = 0; i<TemPro->length; i++)
	    		{
	    			if(strcasecmp(TemPro->option[i].keyword,keyword_found)==0)
	    			{
	    				if(num_opts==2)
	    				{
		    				strcat(TemPro->option[i].msg,"$%^&");
		    				strcat(TemPro->option[i].msg,msg_found);
	    				}
	    				my_flag = 1;
	    			}
	    		}
	    		/* if we don't find this situation, mean that we need to store this keyword and msg, not just msg*/
	    		if(my_flag==0)
	    		{
	    			strcpy(TemPro->option[TemPro->length].keyword,keyword_found);
	    			if(num_opts==2)
	    			{
	    				strcpy(TemPro->option[TemPro->length].msg,msg_found);
	    			}
	    			TemPro->length++;
	    		}
	    	}
	...
	/*some temp designed for the loop*/
	/* start with positive node, then neg node*/
	sfip_node_t* temp_sip_node = sip_head_head;
	sfip_node_t* temp_dip_node = dip_head_head;
	int i = 0;
	int j = 0;
	for(;i<sip_total_count;i++)
	{
		/* the positive sip node finished, then deal with the neg sip node*/
		if(i==sip_head_count)
		{
			temp_sip_node = sip_head_neghead;
		}
		for(;j<dip_total_count;j++)
		{
			if(CurrentNode==NULL)
			{
				printf("now the CurrentNode is NULL,error occurred!\n");
				free(MyHead);
				return -1;
			}
			/* the positive dip node finished, then deal with the neg dip node*/
			if(j==dip_head_count)
			{
				temp_dip_node = dip_head_neghead;
			}
			/* for the match.src_ip*/
			/* prepare for the ip*/
			strcpy(CurrentNode->match.src_ip.ip,temp_sip_node->src_ip);
			/* prepare for the netmask*/
			CurrentNode->match.src_ip.bits = temp_sip_node->ip->bits - 96;
			/* prepare for the flags :  equal to the flags in sfip_node_t structure*/
			CurrentNode->match.src_ip.flags = temp_sip_node->flags;

			/* for the match.dst_ip*/
			/* prepare for the ip*/
			strcpy(CurrentNode->match.dst_ip.ip,temp_dip_node->src_ip);
			/* prepare for the netmask*/
			CurrentNode->match.dst_ip.bits = temp_dip_node->ip->bits - 96;

			/* prepare for the flags :  equal to the flags in sfip_node_t structure*/
			CurrentNode->match.dst_ip.flags = temp_dip_node->flags;

			/* Now try to get the proto*/
			CurrentNode->match.proto =  protocol;

			/* now try to solve the match.flags*/
			CurrentNode->match.flags = test_rtn.flags;

			CurrentNode->match.src_ip_num = sip_total_count;
			CurrentNode->match.dst_ip_num = dip_total_count;

			/* now you need to deal with the src port in CurrentNode->match*/
			CurrentNode->match.sports.length = src_port_list.length;
			CurrentNode->match.sports.port   = src_port_list.port;
			CurrentNode->match.dports.length = dst_port_list.length;
			CurrentNode->match.dports.port   = dst_port_list.port;


			/* Now try to get the rule type */
			CurrentNode->priority = priority;

			/* Now try to get the rule type */
			CurrentNode->acctype = rule_type;



			/* deal with your Proprietary */
			memcpy(&CurrentNode->prop,TemPro,sizeof(Proprietary));

			/* count the number of Ofp Rule Node in the OfpRuleHead*/
			MyHead->count++;
			/* now all  have been done, process the link*/
			CurrentNode->next = (OfpNode)malloc(sizeof(OfpRuleNode));
			CurrentNode = CurrentNode->next;
			memset(CurrentNode,0,sizeof(OfpRuleNode));
			CurrentNode->priority = -1;
			if(CurrentNode == NULL)
			{
				printf("in ParseOfpRule, malloc node error!\n");
				free(MyHead);
				return -1;
			}
			CurrentNode->next = NULL;
			temp_dip_node = temp_dip_node->next;
		}
		j = 0;
		temp_dip_node = dip_head_head;
		temp_sip_node = temp_sip_node->next;
	}
	/* priority ++ for the next time use*/
	priority++;

	/* for my test*/

#ifdef MY_DEBUG
	/* this part I am trying to test whether my option part is right*/
	OfpNode test_option_node;
	test_option_node = MyHead->RuleNode;
	printf("now the length is %d\n",test_option_node->prop.length);
	for(int i = 0; i < test_option_node->prop.length;i++)
	{
		printf("the keyword is %s and the msg is %s\n",test_option_node->prop.option[i].keyword,test_option_node->prop.option[i].msg);
	}
	/* this part I am trying to test whether the other parts are right excpet option part*/
	OfpNode test_node;
	test_node = MyHead->RuleNode;
	for(int i = 0;i<MyHead->count;i++)
	{
		printf("this is number %d node, and the total number is %d.\n",i,MyHead->count);
		if((test_node->match.src_ip.flags&SFIP_NEGATED)&&(test_node->match.dst_ip.flags&SFIP_NEGATED))
		{
			printf("the source ip is !%s and the netmask is %d\n",test_node->match.src_ip.ip,test_node->match.src_ip.bits);
			printf("the dst ip is !%s and the netmask is %d\n",test_node->match.dst_ip.ip,test_node->match.dst_ip.bits);
		}
		else if((!(test_node->match.src_ip.flags&SFIP_NEGATED))&&(test_node->match.dst_ip.flags&SFIP_NEGATED))
		{
			printf("the source ip is %s and the netmask is %d\n",test_node->match.src_ip.ip,test_node->match.src_ip.bits);
			printf("the source ip is !%s and the netmask is %d\n",test_node->match.dst_ip.ip,test_node->match.dst_ip.bits);
		}
		...
		printf("in this node, the src port number is %d\n",test_node->match.sports.length);
		printf("in this node, the src ports are:\n");
		for(int j = 0;j<test_node->match.sports.length;j++)
		{
			if(test_node->match.sports.port[j].type==PORT_OBJECT_PORT)
			{
				printf("%d\t",test_node->match.sports.port[j].port);
			}
			else if(test_node->match.sports.port[j].type==PORT_OBJECT_RANGE)
				printf("%d:%d\t",test_node->match.sports.port[j].port,test_node->match.sports.port[j].hport);
			if((j+1)%5==0)
			{
				printf("\n");
			}
		...
		printf("\n");
		printf("in this node, the dst ports are:\n");
		for(int j = 0;j<test_node->match.dports.length;j++)
		{
			if(test_node->match.dports.port[j].type==PORT_OBJECT_PORT)
			{
				printf("%d\t",test_node->match.dports.port[j].port);
			}
			else if(test_node->match.dports.port[j].type==PORT_OBJECT_RANGE)
				printf("%d:%d\t",test_node->match.dports.port[j].port,test_node->match.dports.port[j].hport);
			else  /* the type is any*/
				printf("%d\t",test_node->match.dports.port[j].port);
			if((j+1)%5==0)
			{
				printf("\n");
		...
		printf("\n");
		printf("now the length of option part is %d\n",test_node->prop.length);
		printf("\n");
...
/****************************************************************************
 * Name   :  OfpToSnort
 * Purpose:  Convert the Ofp rule to snort rule
 * Arguments:OfpHead ofp_head
 * Returns:  char* --- the final rule in snort
 * Notes:    None
 *****************************************************************************/
char* OfpToSnort(OfpHead ofp_head)
{
	...
	char* final_str = NULL;
	final_str = (char*)malloc(sizeof(char)*MAX_SNORTRULE_LENGTH);
	int node_count = ofp_head->count;
	int temp_count = 1;
	OfpNode temp_node_head = ofp_head->RuleNode;
	OfpNode temp_node_tail = temp_node_head;
	OfpNode temp_node_nexthead = temp_node_head;
	while(temp_count<=node_count)
	{
		while(temp_node_nexthead->priority==temp_node_head->priority)
		{
			temp_node_tail = temp_node_nexthead;
			temp_node_nexthead = temp_node_nexthead->next;
			temp_count++;
		}
		/* between the head and tail, there is a full snort rule*/


		/*deal with protocol*/
		int protocol = temp_node_head->match.proto;
		switch(protocol)
		{
			case IPPROTO_TCP:
				strcat(final_str,RULE_PROTO_OPT__TCP);
				break;
			case IPPROTO_UDP:
				strcat(final_str,RULE_PROTO_OPT__UDP);
				break;
			case IPPROTO_ICMP:
				strcat(final_str,RULE_PROTO_OPT__ICMP);
				break;
			case ETHERNET_TYPE_IP:
				strcat(final_str,RULE_PROTO_OPT__IP);
				break;
			default:
				printf("The protocol is not right!\n");
				return NULL;
		}
		char * src_ip_str = (char*)malloc(sizeof(char)*500);
		char * dst_ip_str = (char*)malloc(sizeof(char)*500);
		char* src_port_str = (char*)malloc(sizeof(char)*500);
		char* dst_port_str = (char*)malloc(sizeof(char)*500);
		char* option_str = (char*)malloc(sizeof(char)*1000);
		memset(src_ip_str,'\0',sizeof(char)*500);
		memset(dst_ip_str,'\0',sizeof(char)*500);
		memset(src_port_str,'\0',sizeof(char)*500);
		memset(dst_port_str,'\0',sizeof(char)*500);
		memset(option_str,'\0',sizeof(char)*1000);
		/*deal with the src ip*/
		if(temp_node_head == temp_node_tail)  /* means that we only have one ip node*/
		{
			if(strcmp(temp_node_head->match.src_ip.ip,"0.0.0.0")==0)
			{
				sprintf(src_ip_str," any/%d",temp_node_head->match.src_ip.bits);
			}
			else if(temp_node_head->match.src_ip.flags&&SFIP_NEGATED)
			{
				sprintf(src_ip_str," !%s/%d",temp_node_head->match.src_ip.ip,temp_node_head->match.src_ip.bits);
			}
			else
			{
				sprintf(src_ip_str," %s/%d",temp_node_head->match.src_ip.ip,temp_node_head->match.src_ip.bits);
			}
			strcat(final_str,src_ip_str);
		}
		else     /*means that we have multiply ip*/
		{
			src_ip_str[0] = ' ';
			src_ip_str[1] = '[';
			int src_ip_total = temp_node_head->match.src_ip_num;
			int dst_ip_total = temp_node_head->match.dst_ip_num;
			OfpNode temp_node = temp_node_head;
			if(dst_ip_total==1)
			{
				for(int i = 1;i <= src_ip_total; i++)
				{
					if(strcmp(temp_node->match.src_ip.ip,"0.0.0.0")==0)
					{
						if(i == 1)
							sprintf(src_ip_str,"%sany/%d",src_ip_str,temp_node->match.src_ip.bits);
						else
							sprintf(src_ip_str,"%s,any/%d",src_ip_str,temp_node->match.src_ip.bits);
					}
					else if(temp_node->match.src_ip.flags&&SFIP_NEGATED)
					{
						if(i == 1)
							sprintf(src_ip_str,"%s!%s/%d",src_ip_str,temp_node->match.src_ip.ip,temp_node->match.src_ip.bits);
						else
							sprintf(src_ip_str,"%s,!%s/%d",src_ip_str,temp_node->match.src_ip.ip,temp_node->match.src_ip.bits);
					}
					else
			...
			{
				    for(int i = 1;i < src_ip_total*dst_ip_total;i++)
					{
						if(i % dst_ip_total == 1)
						{
							if(strcmp(temp_node->match.src_ip.ip,"0.0.0.0")==0)
			...
			int src_str_len = strlen(src_ip_str);
			src_ip_str[src_str_len] = ']';
			strcat(final_str,src_ip_str);
		}



		/* deal with the src port*/
		int src_port_length = temp_node_head->match.sports.length;
		MyPort *src_port_entry = temp_node_head->match.sports.port;
		if(src_port_entry[0].type!=PORT_OBJECT_PORT&&src_port_entry[0].type!=PORT_OBJECT_RANGE)  /*the port type is any*/
		{
			sprintf(src_port_str," %s","any");
		}
		else
		{
			src_port_str[0] = ' ';
			src_port_str[1] = '[';
			for(int j = 0;j<src_port_length;j++)
			{
				if(src_port_entry[j].type==PORT_OBJECT_PORT)
				{
					if(j==0)
						sprintf(src_port_str,"%s%d",src_port_str,src_port_entry[j].port);
					else
						sprintf(src_port_str,"%s,%d",src_port_str,src_port_entry[j].port);
				}
				else if(src_port_entry[j].type==PORT_OBJECT_RANGE)
				{
					if(j==0)
						sprintf(src_port_str,"%s%d:%d",src_port_str,src_port_entry[j].port,src_port_entry[j].hport);
					else
						sprintf(src_port_str,"%s,%d:%d",src_port_str,src_port_entry[j].port,src_port_entry[j].hport);
				}
			}
			int src_port_str_length = strlen(src_port_str);
			src_port_str[src_port_str_length] = ']';
		}
		strcat(final_str,src_port_str);




		/* deal with the direction */
		int rtn_flags = temp_node_head->match.flags;
		if(rtn_flags & BIDIRECTIONAL)
		{
			strcat(final_str," <>");
		}
		else
		...
		/* deal with the dst ip*/
		if(temp_node_head == temp_node_tail)  /* means that we only have one ip node*/
		{
			if(strcmp(temp_node_head->match.dst_ip.ip,"0.0.0.0")==0)
			{
				sprintf(dst_ip_str," any/%d",temp_node_head->match.dst_ip.bits);
			}
			else if(temp_node_head->match.dst_ip.flags&&SFIP_NEGATED)
			{
				sprintf(dst_ip_str," !%s/%d",temp_node_head->match.dst_ip.ip,temp_node_head->match.dst_ip.bits);
			}
			else
			{
				sprintf(dst_ip_str," %s/%d",temp_node_head->match.dst_ip.ip,temp_node_head->match.dst_ip.bits);
			}
			strcat(final_str,dst_ip_str);
		}
		else     /*means that we have multiply dst ip*/
		{
			dst_ip_str[0] = ' ';
			dst_ip_str[1] = '[';
			int dst_ip_total = temp_node_head->match.dst_ip_num;
			OfpNode temp_node = temp_node_head;
			for(int i = 1;i <= dst_ip_total;i++)
			{
				if(strcmp(temp_node->match.dst_ip.ip,"0.0.0.0")==0)
				{
					if(i == 1)
						sprintf(dst_ip_str,"%sany/%d",dst_ip_str,temp_node->match.dst_ip.bits);
					else
						sprintf(dst_ip_str,"%s,any/%d",dst_ip_str,temp_node->match.dst_ip.bits);
				}
				else if(temp_node->match.dst_ip.flags&&SFIP_NEGATED)
				{
					if(i == 1)
						sprintf(dst_ip_str,"%s!%s/%d",dst_ip_str,temp_node->match.dst_ip.ip,temp_node->match.dst_ip.bits);
					else
						sprintf(dst_ip_str,"%s,!%s/%d",dst_ip_str,temp_node->match.dst_ip.ip,temp_node->match.dst_ip.bits);
				}
				else
			...
			int dst_str_len = strlen(dst_ip_str);
			dst_ip_str[dst_str_len] = ']';
			strcat(final_str,dst_ip_str);
		}



		/* deal with the dst port*/
		int dst_port_length = temp_node_head->match.dports.length;
		MyPort *dst_port_entry = temp_node_head->match.dports.port;
		if(dst_port_entry[0].type!=PORT_OBJECT_PORT&&dst_port_entry[0].type!=PORT_OBJECT_RANGE)  /*the port type is any*/
		{
			sprintf(dst_port_str," %s","any");
		}
		else
		{
			dst_port_str[0] = ' ';
			dst_port_str[1] = '[';
			for(int j = 0;j<dst_port_length;j++)
			{
				if(dst_port_entry[j].type==PORT_OBJECT_PORT)
				{
					if(j==0)
						sprintf(dst_port_str,"%s%d",dst_port_str,dst_port_entry[j].port);
					else
						sprintf(dst_port_str,"%s,%d",dst_port_str,dst_port_entry[j].port);
				}
				else if(dst_port_entry[j].type==PORT_OBJECT_RANGE)
				{
					if(j==0)
						sprintf(dst_port_str,"%s%d:%d",dst_port_str,dst_port_entry[j].port,dst_port_entry[j].hport);
					else
						sprintf(dst_port_str,"%s,%d:%d",dst_port_str,dst_port_entry[j].port,dst_port_entry[j].hport);
				}
			}
			int dst_port_str_length = strlen(dst_port_str);
			dst_port_str[dst_port_str_length] = ']';
		}
		strcat(final_str,dst_port_str);




		/* deal with the option part*/
		Proprietary temp_prop = temp_node_head->prop;
		if(temp_prop.flags==1)  /* prove that we have option part*/
		{
			option_str[0] = ' ';
			option_str[1] = '(';
			int prop_num = temp_prop.length;
			for(int i = 0;i<prop_num;i++)
			{
				char **toks = NULL;
				int num_toks;
				toks = mSplit(temp_prop.option[i].msg, "$%^&", 0, &num_toks, 0);
				for(int j = 0;j<num_toks;j++)
				{
					sprintf(option_str,"%s%s:%s;",option_str,temp_prop.option[i].keyword,toks[j]);
				}
			}
			int option_str_length = strlen(option_str);
			option_str[option_str_length] = ')';
		}
		strcat(final_str,option_str);

		temp_node_head = temp_node_nexthead;
		temp_node_tail = temp_node_head;

		/* free the useless space*/
		free(src_ip_str);
		free(dst_ip_str);
		free(src_port_str);
		free(dst_port_str);
