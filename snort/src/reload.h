...
static inline void ReloadAdjust(bool idle, time_t tv_sec)
{
    ...
    if (rae)
    {
        ...
        if (rae != snort_conf->raCurrentEntry)
        {
            ...
            LogMessage("Adjusting %s during reload ( Started at : (%lu) milliseconds.\n", rae->raName, 
        ...
        if (rae->raFunc(idle, rae->raPolicyId, rae->raUserData))
        {
            ...
            LogMessage("Finished adjusting memory for %s : Total calls (packet loop=%u, idle loop=%u), Elapsed time = (%lu) milliseconds.\n",
