...
parse_line(char *buff)
{
	...
	if ((token = strtok(buff, w_space)) == NULL)
		...
		parse_ushort(&Config.Port.icp);
	else if (!strcmp(token, "ofmb_port"))
		parse_ushort(&Config.Port.ofmb);
#if USE_HTCP
...
dump_config(StoreEntry *entry)
{
	...
	dump_ushort(entry, "ofmb_port", Config.Port.ofmb);
#if USE_HTCP
