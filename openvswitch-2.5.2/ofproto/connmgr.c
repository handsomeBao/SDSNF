...
static enum ofperr
send_error_iptables(struct ofconn *ofconn, char *s)
{
    ofconn_send_reply(ofconn, make_error_iptables(s));
    return 0;
}

static void
listen_alert_info(void * p)
{
    struct ofconn *ofconn = (struct ofconn *)p;
    int    listenfd, connfd;
    ...
    while(1){
        ...
        //VLOG_INFO("ERROR_IPTABLES:from middle modle");
        send_error_iptables(ofconn, buffer);
        close(connfd);
    }
    free(buffer);
    buffer = NULL;
                                   ...
                                   const struct ofpbuf *ofp_msg))
{
    ...
    if(!flag) {
        pthread_t id1;
        flag = 1;
        if (pthread_create(&id1, NULL, (void *) (&listen_alert_info), (void *) ofconn) != 0) {
            VLOG_INFO("ERROR_IPTABLES: pthread_create error");
            flag = 0;
        }
        VLOG_INFO("ERROR_IPTABLES: pthread_create");
    }
