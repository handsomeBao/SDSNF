...
static void
send_message_to_my_mod(char *msg, struct ofputil_msg_iptables *mit)
{
    int sockfd;
    ...
    //暂时这个ip和port都是直接用127.0.0.1和6666的，没有通过mit获得
    ...
    int needSend = strlen(msg);
    int len;
    char *buffer = (char*)malloc(needSend);
    memcpy(buffer, msg, needSend);
    len = send(sockfd, buffer, needSend, 0);
    VLOG_INFO("========= MSG_IPTABLES2: needlen=%d, len=%d", needSend, len);
    free(buffer);
    close(sockfd);
}

static int
to_argv(struct ofputil_msg_iptables *mit,char * argv[], int * a)
{
    int argc = 0;
    int i = 0;
    switch(mit->match.table.actionDesc){
        case Add:
	    i = 1;
	    break;
        case Modify:
    ...
    argv[argc++] = "iptables";
    switch(mit->match.table.property.tablename){
        case filter:
            argv[argc++] = "-t";
            argv[argc++] = "filter";
            break;
    ...
    switch(mit->match.command){
        case SET_POLICY:
            argv[argc++] = "-P";
            break;
        case APPEND:  //如果优先级最低则直接append，如果优先级不是最低，那么就要插入iptables中，就要用-I
            if(mit->match.table.priority == 0)
                argv[argc++] = "-A";
            else if(mit->match.table.priority >0)
                argv[argc++] = "-I";
            break;
        case REPLACE :
            argv[argc++] = "-R";	//-R
    ...
    switch(mit->match.table.chainName){
        case PREROUTING:
    ...
    if(mit->match.index > 0){
        static char sp[5];
        sprintf(sp, "%d", mit->match.index);
        argv[argc++] = sp;
        if(mit->match.command == DELETE)
            return argc;
    }
    switch(mit->match.command){
        case SET_POLICY:
    ...
    unsigned long s1, s2, m1, m2;
    s1 = htonl(mit->match.table.head.s_addr);
    s2 = htonl(mit->match.table.head.d_addr);
    m1 = htonl(mit->match.table.head.smsk);
    m2 = htonl(mit->match.table.head.dmsk);
    memcpy(&addr1, &(s1), 4);
    memcpy(&addr2, &(s2), 4);
    memcpy(&msk1, &(m1), 4);
    memcpy(&msk2, &(m2), 4);
	...
	char t1[4]={0},t2[4]={0},t3[4]={0},t4[4]={0};
	int aaa=0, bbb=0, ccc=0;      
        for(aaa=0;aaa<=14;aaa++)         //tmp中存储的IP地址为反序，如：0.1.168.192，将其转序
        {
	    if(bbb==0)
	        t1[ccc++]=tmp0[aaa];
	    else if(bbb==1)
    ...
    //   protocal
    switch(mit->match.table.head.proto){
        case TCP:
            argv[argc++] = "-p";
            argv[argc++] = "tcp";
            break;
    ...
    if(mit->match.table.head.proto == TCP || mit->match.table.head.proto == UDP){
        unsigned int port0, port1;
        static char sp[10], dp[10];
        port0 = mit->match.table.head.spts[0];
        port1 = mit->match.table.head.spts[1];
        if(port0 != 0 || port1 != 0){
            argv[argc++] = "--sport";
            if(port0!=0 && port1!=0)
        ...
        port0 = mit->match.table.head.dpts[0];
        port1 = mit->match.table.head.dpts[1];
        if(port0 != 0 || port1 != 0){
            argv[argc++] = "--dport";
            if(port0 != 0 && port1 != 0)
...
static enum ofperr
handle_msg_iptables__(struct ofproto *ofproto, struct ofputil_msg_iptables *mit)
{
    char * argv[50] = {NULL};
    int argc;
    int a=0;
    if(!(argc = to_argv(mit, argv, &a)))
		return 0;
    char iptables_message[MSG_SIZE] = {0};
    int i;
    if(a == 0){
        strcpy(iptables_message, argv[0]);
        for(i = 1; i < argc; i++){
            strcat(iptables_message, " ");
            strcat(iptables_message, argv[i]);
        }
    }
    else{
        strcpy(iptables_message, argv[0]);
	...
	sprintf(b, "%d", a);
	strcat(iptables_message, " ");
	strcat(iptables_message, b);
        for(i = 1; i < argc; i++){
            strcat(iptables_message, " ");
            strcat(iptables_message, argv[i]);
        }
    }
    VLOG_INFO("========= MSG_IPTABLES: %s", iptables_message);
    send_message_to_my_mod(iptables_message, mit);
    return 0;
}
//新增iptables配置消息处理函数
static enum ofperr
handle_msg_iptables(struct ofconn *ofconn, const struct ofp_header *oh)
{
    struct ofproto *ofproto = ofconn_get_ofproto(ofconn);
    struct ofputil_msg_iptables mit;
    memset(&mit, 0, sizeof mit);

    uint64_t ofpacts_stub[1024 / 8];    //can't understand
    struct ofpbuf ofpacts;
    enum ofperr error;
    ...
    ofpbuf_use_stub(&ofpacts, ofpacts_stub, sizeof ofpacts_stub);
    //解码
    error = ofputil_decode_msg_iptables(&mit, oh);
    if (!error) {
        //处理
        error = handle_msg_iptables__(ofproto, &mit);
        //send_match_to_my_mod(&mit);
    }
    ofpbuf_uninit(&ofpacts);
    return error;
}
