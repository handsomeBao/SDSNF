...
uint32_t mf_set_mitmatch(enum oxm_mit_match_fields mf,
                         ...
                         struct handle *match){
    switch (mf) {
        case MITP_TABLE_NAME:
            match->table.property.tablename  = (TableName)value->u8;
            VLOG_INFO("IPT  field = %d,u8 = %x!!!", mf, match->table.property.tablename);
            break;
        case MITP_COMMAND:
            match->command = (CommandList)value->u8;
            VLOG_INFO("IPT  field = %d,u8 = %x!!!",mf,match->command);
            break;
        case MITP_PRIORITY:
            match->table.priority = value->u8;
            VLOG_INFO("IPT  field = %d,u8 = %x!!!",mf,match->table.priority);
            break;
        case MITP_CHAIN_NAME:
            match->table.chainName = (ChainName)value->u8;
            VLOG_INFO("IPT  field = %d,u8 = %x!!!",mf,match->table.chainName);
            break;
        case MITP_ACTION:
            match->table.actionDesc = (ActionDesc)value->u8;
            VLOG_INFO("IPT  field = %d,u8 = %x!!!",mf,match->table.actionDesc);
            break;
        case MITP_INDEX:
            match->index = ntohs(value->be16);
            VLOG_INFO("IPT  field = %d,u16 = %x!!!",mf,match->index);
            break;
        case MITP_SOURCE_IP:
            match->table.head.s_addr = value->be32;
            VLOG_INFO("IPT  field = %d,u32 = %x!!!",mf,match->table.head.s_addr);
            break;
        case MITP_DEST_IP:
            match->table.head.d_addr = value->be32;
            VLOG_INFO("IPT  field = %d,u32 = %x!!!",mf,match->table.head.d_addr);
            break;
        case MITP_SOURCE_IP_MASK:
            match->table.head.smsk = value->be32;
            VLOG_INFO("IPT  field = %d,u32 = %x!!!",mf,match->table.head.smsk);
            break;
        case MITP_DEST_IP_MASK:
            match->table.head.dmsk = value->be32;
            VLOG_INFO("IPT  field = %d,u32 = %x!!!",mf,match->table.head.dmsk);
            break;
        case MITP_PROTO:
            match->table.head.proto = (ProtoType)value->u8;
            VLOG_INFO("IPT  field = %d,u8 = %x!!!",mf,match->table.head.proto);
            break;
        case MITP_SOURCE_PORT1:
            match->table.head.spts[0] = ntohs(value->be16);
            VLOG_INFO("IPT  field = %d,u16 = %x!!!",mf,match->table.head.spts[0]);
            break;
        case MITP_SOURCE_PORT2:
            match->table.head.spts[1] = ntohs(value->be16);
            VLOG_INFO("IPT  field = %d,u16 = %x!!!",mf,match->table.head.spts[1]);
            break;
        case MITP_DEST_PORT1:
            match->table.head.dpts[0] = ntohs(value->be16);
            VLOG_INFO("IPT  field = %d,u16 = %x!!!",mf,match->table.head.dpts[0]);
            break;
        case MITP_DEST_PORT2:
            match->table.head.dpts[1] = ntohs(value->be16);
            VLOG_INFO("IPT  field = %d,u16 = %x!!!",mf,match->table.head.dpts[1]);
            break;
        default:
            VLOG_DBG_RL(&rl,"IPT  mf_set_mitmatch error one");
    }
    return 0;
}
static enum ofperr
