                       ...
                       union mf_value *value)
{
    ...
    void *copy_dst;
    if (b->size < 4) {
        VLOG_DBG_RL(&rl, "IPT nx_pull_mitmatch_entry error b->size!!!");
        return OFPERR_OFPBMC_BAD_LEN;
    }
    header = ((uint64_t) ntohl(get_unaligned_be32(b->data))) << 32;
    ofpbuf_pull(b, nxm_header_len(header));    //oxm头，4字节

    *mf = (enum oxm_mit_match_fields)((header >> 41) & 0x7f); //get oxm_field

    payload_len = nxm_payload_len(header);
    payload = ofpbuf_try_pull(b, payload_len);  //提取oxm消息体，oxm_length
    if (!payload) {
        VLOG_DBG_RL(&rl, "IPT nx_pull_mitmatch_entry error payload: field = %d!!!",*mf);
        return OFPERR_OFPBMC_BAD_LEN;
    }
    copy_dst = value;
    memcpy(copy_dst, payload, payload_len);
    //VLOG_INFO("IPT  field = %d,32 = %x,16 = %x!!!",*mf,value->be32,value->be16);
    return 0;
}
...
nx_pull_mit_raw(const uint8_t *p, unsigned int match_len,
    ...
    while (b.size) {
        ...
        error = nx_pull_mitmatch_entry(&b, &mf, &value);
        if (error) {
            VLOG_DBG_RL(&rl, "IPT nx_pull_mit_raw error one, parsing OXM at offset %"PRIdPTR" "
                    "within match", pos - p);
            return error;
        }
        else {
            mf_set_mitmatch(mf, &value, match);
        }
    }
    return 0;
}
...
enum ofperr oxm_pull_mit_match(struct ofpbuf *b,
                                      struct handle *match)
{
    struct ofp11_match_header *omh = b->data;
    uint8_t *p;
    uint16_t match_len; //整个匹配条件区的长度
    match_len = ntohs(omh->length);
    //取出b中的所匹配信息，ROUND_UP实现整数字节提取
    p = ofpbuf_try_pull(b, ROUND_UP(match_len, 8));
    if (!p) {
        VLOG_DBG_RL(&rl, "IPT oxm_pull_mit_match error one: try_pull error!!!");
        return OFPERR_OFPBMC_BAD_LEN;
    }
    return nx_pull_mit_raw(p + sizeof *omh, match_len - sizeof *omh,
                           match);
}
