...
enum ofperr
ofputil_pull_mit_match(struct ofpbuf *buf, struct handle *match,
                       uint16_t *padded_match_len)
{
    struct ofp11_match_header *omh = buf->data;  //match头，关于这个ofp11，不知道可不可以用
    uint16_t match_len;
    ...
    return oxm_pull_mit_match(buf, match);
}
                            ...
                            const struct ofp_header *oh)
{
    ...
    struct ofpbuf b;
    enum ofpraw raw;
    const struct ofp13_msg_iptables *ofmit;
    //初始化b，size=allocated=ntohs(oh->length)，data=base=oh
    ofpbuf_use_const(&b, oh, ntohs(oh->length));

    //提取openflow头，assert消息类型
    raw = ofpraw_pull_assert(&b);
    ovs_assert(raw == OFPRAW_OFPT13_MSG_IPTABLES);

    //提取出b中消息的ofmit部分，即OFPTYPE_MSG_IPTABLES头
    ofmit = ofpbuf_pull(&b, sizeof *ofmit);

    //解析match的数据
    error = ofputil_pull_mit_match(&b, &mit->match, NULL);
    if (error){
        VLOG_FATAL("IPT ofputil_decode_msg_iptables error one: match error!!!");
        return error;
    }

    //获取其他消息体信息
    mit->dip = ofmit->dip;
    mit->dport = ofmit->dport;
    return 0;
}

int put_match_from_rt(ruletable *s, struct ofpbuf *b){
    struct oxmh{
        uint16_t class;
        uint8_t field;
        uint8_t length;
    ...
    int match_len = 0;
    //enum ofp12_oxm_class class = OFPXMC12_IPTABLES_BASIC;
    oxmh1.class = htons(0x8002);
    if(s->property.tablename){
        //VLOG_INFO("\nERROR_IPTABLES: table_name = %d",s->property.tablename);
        oxmh1.field = (44 << 1);
        oxmh1.length = 1;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->property.tablename), oxmh1.length);
        match_len += 5;
    }
    if(s->chainName){
        //VLOG_INFO("ERROR_IPTABLES: chain_name = %d",s->chainName);
        oxmh1.field = (47 << 1);
        oxmh1.length = 1;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->chainName), oxmh1.length);
        match_len += 5;
    }
    if(s->actionDesc){
        oxmh1.field = (48 << 1);
        oxmh1.length = 1;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->actionDesc), oxmh1.length);
        match_len += 5;
    }
    if(s->priority){
        oxmh1.field = (46 << 1);
        oxmh1.length = 1;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->priority), oxmh1.length);
        match_len += 5;
    }
    ...
    if(s->head.s_addr){
        ...
        oxmh1.length = 4;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->head.s_addr), oxmh1.length);
        match_len += 8;
    }
    if(s->head.d_addr){
        oxmh1.field = (51 << 1);
        oxmh1.length = 4;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->head.d_addr), oxmh1.length);
        match_len += 8;
    }
    if(s->head.smsk){
        oxmh1.field = (52 << 1);
        oxmh1.length = 4;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->head.smsk), oxmh1.length);
        match_len += 8;
    }
    if(s->head.dmsk){
        oxmh1.field = (53 << 1);
        oxmh1.length = 4;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->head.dmsk), oxmh1.length);
        match_len += 8;
    }
    if(s->head.proto){
        oxmh1.field = (54 << 1);
        oxmh1.length = 1;
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&(s->head.proto), oxmh1.length);
        match_len += 5;
    }
    unsigned short port;
    if(s->head.spts[0]){
        oxmh1.field = (55 << 1);
        oxmh1.length = 2;
        port = htons(s->head.spts[0]);
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&port, oxmh1.length);
        match_len += 6;
    }
    if(s->head.spts[1]){
        oxmh1.field = (56 << 1);
        oxmh1.length = 2;
        port = htons(s->head.spts[1]);
        ofpbuf_put(b, (void *)&oxmh1, sizeof(oxmh1));
        ofpbuf_put(b, (void *)&port, oxmh1.length);
        match_len += 6;
    }
...
int ipt_put_match(ruletable *s, struct ofpbuf *b){
    int match_len;
    struct ofp11_match_header *omh;
    size_t start_len = b->size;
    match_len = sizeof *omh;
    ofpbuf_put_zeros(b, match_len);
    match_len += put_match_from_rt(s,b);
    ...
    //VLOG_INFO("ERROR_IPTABLES: match_len = %d",match_len);
    return match_len;
}
...
{
    struct ofp13_error_iptables eip;
    struct ofpbuf *msg;
    ...
    s += 4;
    eip.type = htons(type);
    eip.code = htons(length);
    //VLOG_INFO("ERROR_IPTABLES: type = %d, len = %d",type,length);
    if(type == 0 || type == 1){
        msg = ofpraw_alloc(OFPRAW_OFPT13_ERROR_IPTABLES, 0x04, 0);
        ofpbuf_put(msg, (void *)&eip, sizeof(eip));
        ofpbuf_put(msg, (void *)s, length - 4);
    }
    else if(type == 2){
        ruletable rt;
        memcpy(&rt, (void *)s, sizeof(ruletable));
        msg = ofpraw_alloc(OFPRAW_OFPT13_ERROR_IPTABLES, 0x04, 0);
        ofpbuf_put(msg, (void *)&eip, sizeof(eip));
        ipt_put_match(&rt, msg);

    }
    else{
        VLOG_INFO("ERROR_IPTABLES: wrong type");
    }
